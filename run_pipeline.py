...
def main():
    ...
    case_id = slugify(onenote_section) if onenote_section else args.case_id
    llm_mode = args.mode
    ...
    # Aggregates output path (only if onenote_section provided)
    agg_path = ""
    if onenote_section:
        agg_slug = re.sub(r"[^a-z0-9]+", "_", onenote_section.lower()).strip("_")
        agg_path = str((ROOT / "process" / "onenote_aggregates" / notebook_name / f"{agg_slug}.json").resolve())
    ...
    if onenote_section:
        run([
            ...
            "--section", onenote_section,
            "--out", str((ROOT / "process" / "onenote_aggregates")),
        ])
    ...
    run([
        ...
        "--case-id", case_id,
        "--out", str(PLANS_ROOT),
        "--onenote-section", onenote_section,
    ])
