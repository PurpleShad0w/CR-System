...
def _rgb(value): return ImageColor.getrgb(value)
...
def render_shadow225_from_image(input_png, output_png, cfg=None):
    cfg=cfg or {}; input_png=Path(input_png); output_png=Path(output_png); output_png.parent.mkdir(parents=True,exist_ok=True)
    img_l=Image.open(input_png).convert('L'); ink=np.array(img_l)<int(cfg.get('threshold',245)); h,w=ink.shape
    ...
    mask_img=Image.fromarray((ink.astype(np.uint8)*255),'L').filter(ImageFilter.GaussianBlur(float(cfg.get('blur_radius',1.15))))
    shadow_mask=np.array(mask_img)/255.0; yy,xx=np.where(shadow_mask>0.03); values=shadow_mask[yy,xx]
    for i in range(steps,0,-1):
        t=i/steps; ox=int(round(depth*sx*t)); oy=int(round(depth*sy*t)); alpha=(a1+(a0-a1)*t)*values
