...
def render_shadow225_from_image(input_png: str | Path, output_png: str | Path, cfg: dict[str, Any] | None = None) -> Path:
    ...
    ink = _dilate(ink, int(cfg.get("top_dilate_px", 0)))
    h, w = ink.shape
    ...
    # Shadow-only: progressive grey casts, blurred very slightly. No vertical/extruded sides.
    ...
    # Top black linework remains original and crisp.
    yy, xx = np.where(ink)
    ...
    if bool(cfg.get("crop_to_content", True)):
        ...
        out = _crop(out, gray < 250, int(cfg.get("crop_margin_px", 34)))
    out.save(output_png)
    return output_png
