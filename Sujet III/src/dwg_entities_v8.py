...
def _arc(center: Any, radius: float, start_deg: float, end_deg: float, n: int = 48) -> list[tuple[float, float]]:
...
def _circle(center: Any, radius: float, n: int = 112) -> list[tuple[float, float]]:
...
def _entity_points(entity: Any) -> tuple[list[tuple[float, float]], bool]:
    ...
    try:
        ...
        if t == "ELLIPSE":
            return [_xy(p) for p in entity.flattening(distance=1.2)], bool(getattr(entity, "closed", False))
        if t == "SPLINE":
            return [_xy(p) for p in entity.flattening(distance=1.2)], False
    except Exception:
        return [], False
    return [], False
...
def _extra(entity: Any) -> dict[str, Any]:
    ...
    try:
        if entity.dxftype() == "ARC":
            ...
            out["arc_angle"] = abs(end - start) % 360
            out["arc_radius"] = float(entity.dxf.radius)
        elif entity.dxftype() == "CIRCLE":
            out["arc_angle"] = 360.0
            out["arc_radius"] = float(entity.dxf.radius)
        elif entity.dxftype() == "INSERT":
            out["block_name"] = str(getattr(entity.dxf, "name", ""))
    except Exception:
        pass
    return out
...
def extract_entities_df(path: str | Path, converter: Optional[dict[str, Any]] = None, include_blocks: bool = True) -> pd.DataFrame:
    ...
    for entity in doc.modelspace():
        if entity.dxftype() == "INSERT" and include_blocks:
            ...
            # v8: on rend les virtual entities, plus fidèles que le bloc nu.
            try:
    ...
    for col in ["arc_angle", "arc_radius", "block_name"]:
