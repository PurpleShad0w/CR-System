...
def main():
    p=argparse.ArgumentParser(description='v12 mesh cleanup puis shadow-only 2.25D')
    p.add_argument('input_png')
    p.add_argument('--rules',default='config/default_rules_shadow225_v12.yaml')
    p.add_argument('--out-2d',default='output/rendered_clean_v12_mesh_2d.png')
    p.add_argument('--out-225d',default='output/rendered_clean_v12_mesh_shadow225.png')
    a=p.parse_args(); rules=load_yaml(a.rules,{})
    cleaned=clean_meshes(a.input_png,a.out_2d,rules.get('mesh_cleanup',{}) or {})
